...
<html>
  <head>
    ...
    <title>view all question  page</title>
  ...
  <body >
  <div>
  <span><font style="font-size: 20px;">正在查看${pno}试卷</font></span>
  </div>
							...
							<ul>
								...
								<li id="qu_${status.count}">
	                                   ...
	                                   <c:if test="${stuanswer.qno==question.qno}">
	                                   		<font style="margin-left: 80px;">我的答案：</font><span>${stuanswer.stuanswer}</span>
	                                   </c:if>
	                                   </c:forEach> 
