...
<link rel="stylesheet" href="<%=basePath%>css/colour.css" type="text/css"
  ...
  <body>
 	<form action="<%=basePath%>PaperQuerySvl?operate=querypaper" method="post">
			...
			<table>
				...
				<c:forEach var="paper" items="${papers}">
					<tr>
						...
						<td>${paper.ptitle}</td>
						<td>${paper.point}</td>
						<td style="text-align:center"><a href="<%=basePath%>PaperSvl?operate=showPaperques&&pno=${paper.pno}&ptitle=${paper.ptitle}" class="edit" target="_parent">Begin Exam</a></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
